...
This ROS node is used for sending and cancelling the goal using the action server for, PlanningAction count the number of reached and cancelled goals through the /counter service and for publish informations, received by the /odom topic, to the /posvel topic. 
...
def position():
	"""
    	...
    	Returns: 
    	goal(PlanningGoal): the robot's goal
...
def menu(client):
	"""
    	...
    	Args: 
    	client(PlanningAction): action client
...
def main():
	...
	"""
	Function used for starting the action client, for subscribing to /odom topic,
	for synchronizing client and server of the /counter service and for calling the 
	menu function.
